...
void which_error() {
  ...
  case EINVAL: 
    std::cerr << " The socket is already bound to  an  address.   This\n"
              ...
              << "details.\n"; 
    break; 
  case EACCES: std::cerr << " The address is protected, and the user is not\n"
                         << " the super-user.\n"; break; 
  case ENOTSOCK: std::cerr << "Argument is a descriptor for a file, not a\n"
                           << "socket.\n"
                           << "\n"
                           ...
                           << "sockets:\n"; break;
#if 0 
  case EINVAL: std::cerr << " The addrlen is wrong, or the socket was not\n"
                         << " in the AF_UNIX family.\n"; break; 
#endif
  case EROFS: std::cerr << "  The socket inode would reside on a  read-only\n"
                        << "  file system.\n"; break; 
  case EFAULT: std::cerr << " my_addr   points   outside  the  user's\n"
                         << "accessible address space.\n"; break; 
  case ENAMETOOLONG: std::cerr << " my_addr is too long.\n"; break; 
  case ENOENT: std::cerr << " The file does not exist.\n"; break; 
  case ENOMEM: std::cerr << " Insufficient kernel memory was available.\n";
    break; 
  case ENOTDIR: std::cerr << "A component of the path prefix is not a\n"
                          << "directory.\n"; break; 
#if 0
  case EACCES: std::cerr << " Search  permission  is denied on a component\n"
                         << " of the path prefix.\n"; break; 
#endif
  case ELOOP: 
