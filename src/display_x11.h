...
private:
    ...
    void ASetForeground( unsigned long pix )
      {
...
public:
    ...
    void ASetNewPixmap( Pixmap p )
      {
          pixmap = p;
      }

    void ASetPlainArea( const Area2d & );
    void ASetWindowArea( int width,
                         int height );
...
protected:
    ...
    struct D_XPoints
        ...
        */
        void set_max_size( int num )
          {
              pp.set_max_size( num + 1 );
          }
    };
...
public:
    void draw_point( DisplayObject * d_obj,
                     const RGBcolor & col,
                     const Point2d & );

    DisplayObject * create_point()
      {
          return new D_XPoint;
      }

    void draw_points( DisplayObject * d_obj,
                      const RGBcolor & col,
                      const int num,
                      const Point2d * );
    DisplayObject * create_points( const int num )
      {
          return new D_XPoints( num );
      }

    //the endpoints of the line are absolute coordinates
    void draw_line( DisplayObject * d_obj,
                    const RGBcolor & col,
                    const Line2d & );
    DisplayObject * create_line()
      {
          return new D_XSegment;
      }

    void draw_lines( DisplayObject * d_obj,
                     const RGBcolor & col,
                     const int num,
                     const Line2d * );
    DisplayObject * create_lines( const int num )
      {
          return new D_XSegments( num );
      }

    void draw_circle( DisplayObject * d_obj,
                      const RGBcolor & col,
                      const Circle2d & );
    void fill_circle( DisplayObject * d_obj,
                      const RGBcolor & col,
                      const Circle2d & );
    DisplayObject * create_circle()
      {
          return new D_XArc;
      }

    void draw_circles( DisplayObject * d_obj,
                       const RGBcolor & col,
                       const int num,
                       const Circle2d * );
    void fill_circles( DisplayObject * d_obj,
                       const RGBcolor & col,
                       const int num,
                       const Circle2d * );
    DisplayObject * create_circles( const int num )
      {
          return new D_XArcs( num );
      }


    //specifies an arc with absolute begin in ang1 and absolute!!! end in ang2, the path goes from ang1 to ang2 couterclockwise
    void draw_circlearc( DisplayObject * d_obj,
                         const RGBcolor & col,
                         const CircleArc2d & );
    void fill_circlearc( DisplayObject * d_obj,
                         const RGBcolor & col,
                         const CircleArc2d & );
    DisplayObject * create_circlearc()
      {
          return new D_XArc;
      }

    void draw_circlearcs( DisplayObject * d_obj,
                          const RGBcolor & col,
                          const int num,
                          const CircleArc2d * );
    void fill_circlearcs( DisplayObject * d_obj,
                          const RGBcolor & col,
                          const int num,
                          const CircleArc2d * );
    DisplayObject * create_circlearcs( const int num )
      {
          return new D_XArcs( num );
      }

    //all boudary points of the polygon are treated as absolute coordinates
    void draw_polyline( DisplayObject * d_obj,
                        const RGBcolor & col,
                        const int num,
                        const Point2d * );
    DisplayObject * create_polyline( const int num )
      {
          return new D_XPoints( num );
      }

    void draw_polygon( DisplayObject * d_obj,
                       const RGBcolor & col,
                       const int num,
                       const Point2d * );
    void fill_polygon( DisplayObject * d_obj,
                       const RGBcolor & col,
                       const int num,
                       const Point2d * );
    DisplayObject * create_polygon( const int num )
      {
          return new D_XPoints( num + 1 );
      }

    void draw_string( DisplayObject * d_obj,
                      const RGBcolor & col,
                      const Point2d &,
                      const int len,
                      const char * str );
    DisplayObject * create_string()
      {
          return new D_XPoint;
      }
...
protected:
    ...
    void copy_2_XSegment( XSegment & s,
    ...
    void copy_2_XArc( XArc & a,
                      const CircleArc2d & circlearc ) const
      {
    ...
    void copy_2_XPoints( XPoint * dest,
                         const int num,
    ...
    void copy_2_XPoints( XPoint * dest,
                         ...
                         const Point2d * vec,
                         const Point2d & ) const;
    void copy_2_XSegments( XSegment * dest,
                           const int num,
