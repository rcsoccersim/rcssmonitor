...
void
MenuX11::MyWin::destroy_window( Display * disp )
{
    if ( exists )
    {
...
bool
MenuX11::responsible_for_event( const XEvent & event ) const
{
    if ( buttons.exists
         && buttons.visible
...
bool
MenuX11::process_event( const XEvent & event )
{
    if ( buttons.exists
         && buttons.visible
         && event.xany.window == buttons.win )
...
void
MenuX11::set_background_color( const RGBcolor & col )
{
...
void
MenuX11::set_foreground_color( const RGBcolor & col )
{
    unsigned int pixel = get_color_pixel( col );
...
void
MenuX11::resize( int s_x, int s_y )
{
    if ( s_x == size_x
         && s_y == size_y )
    {
...
bool
MenuX11::set_number_of_buttons( int num )
{
    if ( num <= 0 )
    {
        return false;
    ...
    if ( ! buttons.exists )
    {
        buttons.create_window( disp, win, opener_size_x + 1, 0
                               , buttons_size_x, size_y );
    ...
    if ( ! opener.exists )
    {
        opener.create_window( disp, win, 0, 0,
                              opener_size_x, size_y );
...
bool
MenuX11::set_button_label( int num,
                           const char * lab )
{
...
const char *
MenuX11::get_button_label( int num ) const
{
    if ( num < 0 || num >= num_buttons )
    {
        return 0;
...
bool
MenuX11::set_menu_extended( bool flag )
{
